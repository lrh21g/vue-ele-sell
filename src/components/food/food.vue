      ...
      <div class="food-content">
        ...
        <div class="rating">
          ...
          <ratingselect
            ...
            @select="selectRatingType"
            @toggle="toggleCoutent"></ratingselect>
...
<script>
  ...
  export default {
    ...
    methods: {
      show () {
        this.showFlag = true
        this.selectType = ALL // 初始化选择的评价类型
        this.onlyContent = true // 初始化是否只看有内容的评价
        this.$nextTick(() => {
          if (!this.scroll) {
      ...
      needShow (type, text) {
        if (this.onlyContent && !text) {
          // 查看有内容的评价 并且 没有文本存在，则隐藏，返回 false
          return false
        }
        if (this.selectType === ALL) {
          // 如果评价类型选择为 ALL，则显示，返回 true
          return true
        } else {
          // 如果当前评论的类型 与 选择的类型相同，则返回为 true，否则为 false
